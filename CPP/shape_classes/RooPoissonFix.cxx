...
ClassImp(RooPoissonFix) 



//_____________________________________________________________________________
RooPoissonFix::RooPoissonFix(const char *name, const char *title, 
		       RooAbsReal& _x,
		       RooAbsReal& _mean,
		       Bool_t noRounding) :
...
//_____________________________________________________________________________
 RooPoissonFix::RooPoissonFix(const RooPoissonFix& other, const char* name) :  
...
//_____________________________________________________________________________
Double_t RooPoissonFix::evaluate() const 
{ 
  ...
  if(_protectNegative && mean<0) 
    return 1e-3;
  return TMath::PoissonI(k,mean) ;
} 



//_____________________________________________________________________________
Double_t RooPoissonFix::getLogVal(const RooArgSet* s) const 
{
  // calculate and return the negative log-likelihood of the Poisson                                                                                                                                    
  Double_t expected=mean ;
  Double_t observed=x ;
  Int_t observed_i = TMath::Floor(observed);
  Double_t fact = 0.;
  for(int i=1; i<=observed_i; i++){
    fact += log(i);
  }

  return observed_i*log(expected) - fact - expected;
...
//     cout << "RooPoissonFix::getLogVal(" << GetName() << ") mu=" << expected << " x = " << x << " -log(P) = " << extra << " log(evaluate()) = " << log(prob) << endl ;
//   }
  
//   if (fabs(extra+log(prob))>1) {
//     cout << "RooPoissonFix::getLogVal(" << GetName() << ") WARNING mu=" << expected << " x = " << x << " -log(P) = " << extra << " log(evaluate()) = " << log(prob) << endl ;
//   }
...
//_____________________________________________________________________________
Int_t RooPoissonFix::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* /*rangeName*/) const 
{
...
//_____________________________________________________________________________
Double_t RooPoissonFix::analyticalIntegral(Int_t code, const char* rangeName) const 
{
...
//_____________________________________________________________________________
Int_t RooPoissonFix::getGenerator(const RooArgSet& directVars, RooArgSet &generateVars, Bool_t /*staticInitOK*/) const
{
...
//_____________________________________________________________________________
void RooPoissonFix::generateEvent(Int_t code)
{
