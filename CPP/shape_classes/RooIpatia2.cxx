...
Double_t low_x_LnBK(Double_t nu, Double_t x) {
  return TMath::Log(TMath::Gamma(nu)) + (nu - 1.) * log_de_2 -
         nu * TMath::Log(x);
}
...
Double_t LogEval(Double_t d, Double_t l, Double_t alpha, Double_t beta,
                 Double_t delta) {
  // Double_t d = x-mu;
  ...
  return TMath::Exp(
      logno + beta * d +
      (0.5 - l) * (TMath::Log(alpha) - 0.5 * TMath::Log(thing)) +
      LnBK(l - 0.5,
           alpha * TMath::Sqrt(
                       thing)));  // + TMath::Log(TMath::Abs(beta)+0.0001) );
}

Double_t diff_eval(Double_t d, Double_t l, Double_t alpha, Double_t beta,
                   Double_t delta) {
  // Double_t sq2pi = TMath::Sqrt(2*TMath::ACos(-1));
  ...
  return no * TMath::Power(alpha, ns1) * TMath::Power(thing, l / 2. - 1.25) *
         (-d * alphasq * (BK(l - 1.5, alphasq) + BK(l + 0.5, alphasq)) +
          (2. * (beta * thing + d * l) - d) * BK(ns1, alphasq)) *
         TMath::Exp(beta * d) / 2.;
}
...
Double_t Gauss2F1(Double_t a, Double_t b, Double_t c, Double_t x) {
  if (fabs(x) <= 1) {
    ...
    return ROOT::Math::hyperg(c - a, b, c, 1 - 1 / (1 - x)) /
           TMath::Power(1 - x, b);
  }
  // else {}
}
...
ClassImp(RooIpatia2)
    ...
    RooIpatia2::RooIpatia2(const char* name, const char* title, RooAbsReal& _x,
                           RooAbsReal& _l, RooAbsReal& _zeta, RooAbsReal& _fb,
                           RooAbsReal& _sigma, RooAbsReal& _mu, RooAbsReal& _a,
                           RooAbsReal& _n, RooAbsReal& _a2, RooAbsReal& _n2)
...
Double_t RooIpatia2::evaluate() const {
  ...
  Double_t out = 0.;
  if (zeta != 0.) {
    phi = BK(l + 1., zeta) / BK(l, zeta);  // careful if zeta -> 0. You can
                                           // implement a function for the
                                           // ratio, but carefull again that |nu
                                           // + 1 | != |nu| + 1 so you jave to
                                           // deal wiht the signs
    ...
    if (l <= -1.0) {
      ...
      printf(
          "WARNING: zeta ==0 and l > -1 ==> not defined rms. Changing the "
          "meaning of sigma, but I keep fitting anyway\n");
      delta = sigma;
    }
    delta2 = delta * delta;
    if (d < -asigma) {
      ...
      k2 = beta * k1 -
           cons1 * (l - 0.5) * TMath::Power(phi, l - 1.5) * 2 * asigma / delta2;
      B = -asigma + n * k1 / k2;
      A = k1 * TMath::Power(B + asigma, n);
      out = A * TMath::Power(B - d, -n);
    } else if (d > a2sigma) {
      ...
      k1 = cons1 * TMath::Power(phi, l - 0.5);
      k2 = beta * k1 +
           cons1 * (l - 0.5) * TMath::Power(phi, l - 1.5) * 2. * a2sigma /
               delta2;
      B = -a2sigma - n2 * k1 / k2;
      A = k1 * TMath::Power(B + a2sigma, n2);
      out = A * TMath::Power(B + d, -n2);
...
Int_t RooIpatia2::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars,
...
Double_t RooIpatia2::analyticalIntegral(Int_t code,
                                        const char* rangeName) const {
  // Double_t d = x-mu;
  ...
  if (code == 1) {
    ...
    if (d0 > a2sigma) {
      ...
      B = -a2sigma - n2 * k1 / k2;
      A = k1 * TMath::Power(B + a2sigma, n2);
      return A * (TMath::Power(B + d1, 1 - n2) / (1 - n2) -
                  TMath::Power(B + d0, 1 - n2) / (1 - n2));
    }
    ...
    if (d0 < -asigma) {
      ...
      A = k1 * TMath::Power(B + asigma, n);
      I0 = A * TMath::Power(B - d0, 1 - n) / (n - 1);
      I1a = A * TMath::Power(B + asigma, 1 - n) / (n - 1) -
            stIntegral(-asigma, delta, l);
    } else {
    ...
    if (d1 > a2sigma) {
      ...
      B = -a2sigma - n2 * k1 / k2;
      A = k1 * TMath::Power(B + a2sigma, n2);
      I1b = A * (TMath::Power(B + d1, 1 - n2) / (1 - n2) -
                 TMath::Power(B + a2sigma, 1 - n2) / (1 - n2)) -
            stIntegral(d1, delta, l) + stIntegral(a2sigma, delta, l);
    }
